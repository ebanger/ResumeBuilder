...
function btp_attachment_image_fields_to_edit( $form_fields, $attachment ) {  
	...
	if ( substr( $attachment->post_mime_type, 0, 5 ) == 'image' ){
		...
		$html = '';
		$html .= '<select name="attachments[' . $attachment->ID . '][alt_linking]" id="attachments[' . $attachment->ID .'][alt_linking]">';
...
function btp_loginlogo_capture() {
	...
	if ( !empty( $loginlogo ) ) {
		$out .= '<style type="text/css">';
			$out .= '.login h1 a { background: url(' . $loginlogo . ') center center no-repeat; }';
		$out .= '</style>';
	}
